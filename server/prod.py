...
# A partir de qual coluna começam as medidas
COL_MEDIDAS_INICIO = 6
# Coluna da chave combinada (A para Preparador, C para Operador) – base 0
...
def _normalize_text(s: str) -> str:
    t = _norm(s).lower()
    # tirar acentos de forma simples
    rep = {
        "á": "a", "à": "a", "ã": "a", "â": "a",
        "é": "e", "ê": "e",
        "í": "i",
...
def _parse_range_any(texto: str):
    ...
    Extrai (min, max, unidade) com tolerância:
      ...
      - Se houver token 'mínimo' (ou 'maximo'), respeita só um lado.
    Unidade: melhor-possível (pega sufixo só-letras/símbolos no final).
    """
    if not texto:
    ...
    # Único valor com possíveis tokens de mínimo/máximo
    v = _first_number(s)
...
def _extrair_medidas_pares(row_vals):
    ...
    while True:
        ...
        # tenta faixa, se vazio tenta no título
...
def _extrair_medidas_quartetos(row_vals):
    """Para OPERADOR: (tipo, faixa, periodicidade, instrumento)."""
    medidas = []
    col = COL_MEDIDAS_INICIO
    while True:
